class Orb {
  ...
  PVector getMagneticForce(float current, Wire wire) {
    PVector force = velocity.copy();
    float angle = atan(abs(this.center.y - wire.center.y)/(abs(this.center.x - wire.center.x)));
    force.mult(q);
    force.mult(wire.getB(this, current));
    force.mult(sin(angle));
    force.mult(current);
    return force;
  }
